            ...
            {categories?.map((category) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col justify-center items-center p-5 rounded-md border text-2xl shadow-md hover:text-rose-500 duration-300 ${name === query?.category && "text-rose-500"
                    }`}
                >
                  {logo}
...
export async function getServerSideProps(context) {
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_NEXT_APP_URL}/api/rogReboot?category=${
